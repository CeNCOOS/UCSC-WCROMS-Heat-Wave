...
io=0
tic=time.time()
atime=dt.datetime(2022,10,31)
for offset in thedays:
    print(offset)
    [timearray,temparray]=wcof_load(atime,offset,24,mx,my)
    if io==0:
        bigtime=timearray
...
#    if offset==0:
#        [timearray,temparray]=wcof_load(atime,offset,72,mx,my)
#        bigtime=np.append(bigtime,timearray)
#        bigtemp=np.vstack((bigtemp,temparray))
toc=time.time()
print(toc-tic)
pfile=open('wcofs_october_2022.p','wb')
...
#starttime=atime-dt.timedelta(days=10)
    ...
    if (atime.month==month)&(atime.day==day):
        maxhours=fshour
    else:
        maxhours=nshour
    for zhours in maxhours:
#    for zhours in fshour:            
        if zhours < 10:
            forehour='n00'+str(zhours)
        else:
            if zhours > 24:
                forehour='f0'+str(zhours)
            else:
                forehour='n0'+str(zhours)
