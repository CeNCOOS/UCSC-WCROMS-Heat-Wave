# West coast upper heat content run 
import sys
# sys.path.append("..")
...
#
filemid=43471
for ii in np.arange(0,27): # 92, 67 for 2010
    if ii==0:
    ...
    #
    # ? is 40531, 40535, 40539, 40543, 40547, 40551, 40555, 40559, 40563
    #
    filemid=filemid+4
    thefilename='fwd_'+str(filemid)+'_001_'+filestr+'.nc'
    #thefilename='fwd_43579_001_'+filestr+'.nc'
    #print(thefilename)
    # open the data set
    url='https://oceanmodeling.ucsc.edu:8443/thredds/dodsC/wc12.0_ccsra31_ext_01/posterior/'+thefilename
    #fwd_40531_001_20101221_20101229
    #url='https://oceanmodeling.ucsc.edu:8443/thredds/dodsC/wc12.0_ccsra31_ext_01/posterior/'
    ...
    for j in startavg:
        #avgheat=np.nanmean(westcoast_heat_content[j:j+4,:,:],axis=0) # does the first 4 days
        avgheat=np.nanmean(westcoast_heat_content[j+4:j+8,:,:],axis=0) # try to do the last 4 days of the file
...
dims = ['lat', 'lon','time']
ds = xr.Dataset({'heat_content_2019_100_meters': (dims, the_heat)},
                coords={"time":time,
                        "lon": longitude,
                        "lat": latitude,
                        })
ds.attrs['title'] = "West Coast Upper Ocean Heat Content 2019 - 0-100 meters"
ds.attrs['notes'] = "Created on "+dt.datetime.today().strftime("%Y-%m-%d") + " by flbahr"

fname = "West_Coast_upper_heat_content_2019_last4days_nrt.nc"
