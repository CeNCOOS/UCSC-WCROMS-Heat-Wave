...
startdate1=dt.datetime(2010,4,21)
#startdate1=dt.datetime(2010,4,1)
enddate1=startdate1+dt.timedelta(days=8)
ic=0
#
for ii in np.arange(0,61): # 92, 67 for 2010
    if ii==0:
...
dims = ['lat', 'lon','time']
ds = xr.Dataset({'heat_content_2010_100_meters': (dims, the_heat)},
                coords={"time":time,
                        "lon": longitude,
                        "lat": latitude,
                        })
ds.attrs['title'] = "West Coast Upper Ocean Heat Content 2010 - 0-100 meters"
ds.attrs['notes'] = "Created on "+dt.datetime.today().strftime("%Y-%m-%d") + " by flbahr"

fname = "West_Coast_upper_heat_content_2010_last4days.nc"
